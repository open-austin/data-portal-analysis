...
logging.basicConfig(filename="analyzer.log", filemode="w", level=logging.DEBUG)
...
class DatasetHandler:
    ...
    def _load_json(self, json_filename):
        ...
        logging.info("Loading datasets from %s" % json_filename)
        return data_dict
    ...
    def _multiset_handler(self, data_dict):
        try:
            ...
            logging.critical("""DatasetHandler caught fatal exception:
            No datasets found.""")
            raise KeyError("No datasets found")

        for item in datasets:
            item['snapshot_time'] = self._current_time
        logging.info("DatasetHandler loaded %s datasets at %s"
                     % (len(datasets), self._current_time))
        return datasets
